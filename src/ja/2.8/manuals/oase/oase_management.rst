...
.. tip::
   | 埋め込みデータの中に下記の制御コードや半角記号が含まれる場合、ServiceNowに通知を行う時にエラーとなります。
   | ・ASCIIの制御コード（\\x01～\\x7F）のうち、LF(\\x0a)、CR(\\x0d)を除いたもの
   | ・C1制御コード(\\x80～\\x9F)
   | ・半角記号のうち、ServiceNow 側でエラーとなるもの
   | そのため、通知テンプレートに以下のフィルターを入れることでエラーを回避することができます。
   | 以下は \x01 を削除する例です。必要に応じて、他の制御コードや半角記号についても同様のフィルタを追加してください。
