...
.. tip::
   | 埋め込むデータの中に下記の制御コードが含まれる場合、Exastro IT Automationでエラーとなります。
   | ・ASCIIの制御コード（\\x01～\\x7f）のうち、TAB(\\x09)、LF(\\x0a)、CR(\\x0d)、DEL(\\x7f)を除いたもの
   | そのため、通知テンプレートに以下のフィルターを入れることでエラーを回避することができます。
   | 以下は \\x01 を空文字に変換する例です。必要に応じて、他の制御コードも空文字に変換してください。
