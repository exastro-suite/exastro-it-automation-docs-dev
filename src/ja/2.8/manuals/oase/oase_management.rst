...
.. note::
   ...
   | ・ただし、監視ソフトが返却するレスポンスのぺーロードが、JSON形式のみ、エージェントは処理対象とします。
...
.. note::
  ...
  | ただし、下記のコマンドを実行すると、既に存在するユーザーで作成済みの<Zabbix APIトークン>が使用できなる場合があります。
...
.. note::
  ...
  1. | ブラウザで、:dfn:`Grafana` にサインインします。
     | 　初期設定では、
     | 　・username: admin
     | 　・Password: admin （ただし、既にログインした場合、変更されている可能性があります）
