...
登場する用語については、:ref:`oase_definition` を参照してください。
...
検索方法
^^^^^^^^^
| フィルターの検索方法は、ユニーク、キューイング、グルーピング（期間延長あり／期間延長なし）があります。
...
| グルーピングのフィルターで指定したラベルの値とTTLでグループ化され、グループ毎に１回だけアクションが実行されます。

| また、グルーピングを行う期間を指定することが可能です。
| 検索方法に :menuselection:`グルーピング（期間延長なし）` を選択した場合、グルーピング対象期間を指定し固定することができます。先頭イベントの取得から指定したグルーピング対象期間の間のみ同一グループとして扱います。期間切れ後に取得したイベントは、新しいグループとして評価されます。
| 検索方法に :menuselection:`グルーピング（期間延長あり）` を選択した場合、グルーピング対象期間は固定できません。同一グループのイベントが期間内に続くたびにグルーピング対象期間が延長されます。

| グルーピングのイメージは以下です。

.. figure:: /images/ja/oase/oase/filter_grouping_image_v2-8.png
   :width: 900px
...
.. list-table::
   ...
   * - 検索方法
     - | 検索方法を選択します。
       ...
       | グルーピング：フィルター条件に一致したイベントの中からグルーピング条件で指定したラベルと条件に該当するイベントをそれぞれグルーピングします。イベントがグルーピングされるごとに先頭イベントの期間が延長される「期間延長あり」と、先頭イベントのTTL内でグルーピングされる「期間延長なし」を選択できます。
   ...
   * - 期間（グルーピング対象）
     ...
     - | 最小値：10（秒）
       | 最大値：2147483647（秒）
       | ※期間をイベントのTTLより大きい値に設定した場合、動作は保証できません。
   * - 備考
     - 自由記述欄。レコードの廃止・復活時にも記載可能。
     - ー
     - 手動入力
