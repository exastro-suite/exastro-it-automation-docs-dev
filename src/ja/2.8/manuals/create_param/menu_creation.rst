...
- | パラメータシートの作成パターン
  ...
  | パラメータシートを選択した場合、メニューグループ3つに対してメニュー（パラメータシート）が作成されます。
...
.. note:: | 1つのパラメータシートに追加できる項目の推奨数は100個、上限数は1000個となります。
...
- | パラメータシート作成のために必要な情報を入力します。
  ...
  - | 「対象メニューグループ」枠
    ...
    | 「対象メニューグループを選択」ボタンを押下すると、「対象メニューグループ」選択画面が表示され、使用したいメニューグループを選択、設定することができます。
...
.. _About_linkage_target_items_for_substitution_value_auto-registration_setting:

.. tip:: | **代入値自動登録設定の連携対象項目について**
 | Ansibleドライバでは「文字列（単一行）」、「文字列（複数行）」、「整数」、「小数」、「パスワード」、「リンク」、「ファイルアップロード」が連携されます。「日時」、「日付」、および「プルダウン選択」で「日時」、「日付」を選んだ場合は連携されません。
 | Terraformドライバでは「文字列（単一行）」、「文字列（複数行）」、「整数」、「小数」、「パスワード」、「リンク」が連携されます。「日時」、「日付」、「ファイルアップロード」、および「プルダウン選択」で「日時」、「日付」を選んだ場合は連携されません。
