...
| 尚、Conductor作業実行の完了確認は行いません。完了確認は、 :menuselection:`Conductor  -->  Conductor作業履歴` より行ってください。
...
.. table:: Request bodyについての説明
   ...
   |                                        |                        |      |                  | | 複数メニューが対象で順序性を考慮する必要がある場合は、配列の順番で調整してください。                                          |
   |                                        |                        |      |                  | | Conductor作業実行のみを行う場合は省略してください。                                                                           |
   +----------------+-----------------------+------------------------+------+------------------+-------------------------------------------------------------------------------------------------------------------------------+
   | ※1             | (menu_name_rest)      | メニュー名(REST)       |      | 配列             | | :menuselection:`管理コンソール  -->  メニュー管理` の :menuselection:`メニュー名(Rest)` を指定します。                      |
   |                |                       |                        |      |                  | | 複数レコードが対象で順序性を考慮する必要がある場合は、配列の順番で調整してください。                                          |
   |                +--------+--------------+------------------------+------+------------------+-------------------------------------------------------------------------------------------------------------------------------+
...
| 本APIは、ITAで更新可能なメニューに対してパラメータ適用を行うことができます。
