...
-------------------------------
| Ansible Execution Agentとは、ITAとは独立したAnsible実行専用のサーバーです。
| ITAと連携しPlaybookを実行する為に必要な実行環境(コンテナ)をAnsible-builderで構築し、Ansible-runnerより実行環境(コンテナ)を介しPlaybookを実行するものです。
| Ansible Execution Agentはワークスペース単位に用意します。
| 同じワークスペースに複数のAnsible Execution Agentを用意することで、冗長化構成を組むことができます。

.. image:: /images/ja/diagram/ansible_agent_overview.png
   :width: 6.68819in
   :height: 3.35972in
   :alt: Ansible Execution Agentの構成図

Ansible driverについて
----------------------

| Ansible driverは、ITAの機能で、ITAで登録した作業対象のサーバ・ストレージ・ネットワーク各機器に対し、Ansible Core、Ansible Automation Controller、Ansible Execution Agentのどれを経由するかを選択し、Playbookの処理を自動的に行います。

.. image:: /images/ja/diagram/ansible_overview.png
   :width: 6.68819in
   :height: 3.35972in
   :alt: Exastro概要
...
| グローバル変数として定義することで、設定の重複を防ぎ、変更時の管理を容易にします。

| 複数のPlaybookやタスクで共通して利用する情報を一元的に管理する際に便利です。

| 以下で登録するIaC、テンプレート内でグローバル変数を使用できます。

* :menuselection:`Ansible共通 --> テンプレート管理`
* :menuselection:`Ansible-Legacy --> Playbook素材集`
* :menuselection:`Ansible-Pioneer --> 対話ファイル素材集`
* :menuselection:`Ansible-LegacyRole --> ロールパッケージ管理`
...
| :menuselection:`Ansible共通 --> グローバル変数（センシティブ）管理` から登録する変数です。
| 使用方法は、:menuselection:`グローバル管理` と同様ですが、暗号化して保存します。
| Ansible実行で使用する際には、ansible-vaultを通して使用します。
...
.. tip::
    ...
    * 認証情報: 複数のサーバーで共通利用するユーザー名やパスワードなど。
    * APIキー: 外部サービス連携に必要なAPIキーなど、セキュリティと管理が重要な機微情報など。
...
| 作業実行時に選択された :menuselection:`基本コンソール --> オペレーション一覧` の  :menuselection:`オペレーションID` を変数として扱えます。
...
| 作業実行時に生成される :menuselection:`各モード --> 作業状態確認` の :menuselection:`作業No．` を変数として扱えます。
...
| Conductor実行時に生成される :menuselection:`Conductor --> Conductor作業確認` の :menuselection:`ConductorインスタンスID` を変数として扱えます。
...
| Playbook内でConductor作業ディレクトリパス配下にファイルを作成することで、Conductor内の各Movement間でファイルを共有することができます。
...
.. table::  Playbook素材で変数抜出の対象となる変数の種類と書式
   ...
   |                               |    {{△GBL_xxx△|△フィルタ△}}          |                                                                                          |
   +-------------------------------+--------------------------------------+------------------------------------------------------------------------------------------+
   | グローバル変数                | .. code-block:: yaml                 | 具体値の登録は :menuselection:`Ansible共通 --> グローバル変数（センシティブ）管理`       |
   | （センシティブ）              |                                      | より行います。                                                                           |
   |                               |    {{△GBL_xxx△}}                     |                                                                                          |
   |                               |                                      |                                                                                          |
   |                               |    {{△GBL_xxx△|△フィルタ△}}          |                                                                                          |
...
| :menuselection:`Ansible-Pioneer --> 対話ファイル素材集` の :menuselection:`対話ファイル素材` で変数抜出の対象となる変数の種類と書式は、以下の図のとおりです。
...
| :menuselection:`Ansible-LegacyRole --> ロールパッケージ管理` の :menuselection:`ロールパッケージファイル（ZIP形式）` で変数抜出の対象となる変数の種類と書式は、以下の図のとおりです。
...
.. table::  ロールパッケージファイル（ZIP形式）で変数抜出の対象となる変数の種類と書式
   ...
   +-----------------+------------------------------------------+              +              +              |       +------------------------------------------------------------+
   | グローバル変数  | .. code-block:: yaml                     |              |              |              |       | 具体値の登録は :menuselection:`Ansible共通 --> \           |
   | （センシティブ）|                                          |              |              |              |       | グローバル変数（センシティブ）管理` より行います。         |
   |                 |    {{△GBL_xxx△}}                         |              |              |              |       |                                                            |
   |                 |                                          |              |              |              |       |                                                            |
   |                 |    {{△GBL_xxx△|△フィルタ△}}              |              |              |              |       |                                                            |
   +-----------------+------------------------------------------+              +              +              |       +------------------------------------------------------------+
...
.. tip:: | 各変数名についての補足
    ...
    | vvv: 255バイト以内の半角英数字とアンダースコア（ _ ）
    | xxx: 251バイト以内の半角英数字とアンダースコア（ _ ）
    | メンバー変数は、下記の7文字を除くascii文字(0x20～0x7e)が使用できます。
    | " . [ ] ' \\ :
...
| :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`インベントリファイル追加オプション` と :menuselection:`各モード --> Movement一覧` の :menuselection:`ヘッダセクション` のパラメータ値で変数抜出の対象となる変数の種類と書式は、以下の図のとおりです。
...
#. |  :guilabel:`登録` ボタンより、作業対象となる機器情報の登録を行います。
   ...
   .. figure:: /images/ja/ansible_common/devaicelist/registration_device_list_v2_5.gif
      :width: 800px
      :alt: 登録画面（機器一覧）
...
#. | 登録画面の項目一覧は以下のとおりです。
   ...
   .. table:: 登録画面項目一覧（機器一覧）
      ...
      | le利用\| /Role\ |                 | Ansible Execution Agent\                               |           |              | のとおり        |
      | 情報   | 利用\  |                 | から作業対象へ接続する際の認証方式を選択します。       |           |              |                 |
      |        | 情報   |                 |                                                        |           |              |                 |
      |        |        |                 | + | パスワード認証                                     |           |              |                 |
      |        |        |                 |   | ログインパスワードの入力が必須です。               |           |              |                 |
      |        |        |                 |                                                        |           |              |                 |
      ...
      |        |        |        |        | 接続します。                                           |           |              |                 |
      |        |        |        +--------+--------------------------------------------------------+-----------+--------------+-----------------+
      |        |        |        | winrm\ | WindowsServerに証明書認証でWinRM接続する場合の公開鍵\  | ー        | ファイル選択 | 最大サイズ\     |
      |        |        |        | 公開\  | ファイル「ansible_winrm_cert_pem」を入力します。       |           |              | 100Mバイト      |
      |        |        |        | 鍵ファ\|                                                        |           |              |                 |
      |        |        |        | イル   | ※登録後はダウンロード不可となります。                  |           |              |                 |
      |        |        |        +--------+--------------------------------------------------------+-----------+--------------+-----------------+
      |        |        |        | winrm\ | WindowsServerに証明書認証でWinRM接続する場合の秘密鍵\  | ー        | ファイル選択 | 最大サイズ\     |
      |        |        |        | 秘密\  | ファイル「ansible_winrm_cert_key_pem」を入力します。   |           |              | 100Mバイト      |
      |        |        |        | 鍵ファ\|                                                        |           |              |                 |
      |        |        |        | イル   | ※登録後はダウンロード不可となります。                  |           |              |                 |
      |        +--------+--------+--------+--------------------------------------------------------+-----------+--------------+-----------------+
...
#. | 登録画面の項目一覧は以下のとおりです。
   ...
   .. table:: 登録画面項目一覧（インタフェース情報）
      ...
      | Automation \    |                 |                                                               |           |              | Ansible Automation Controllerの場合に入力必須         |
      | Controller \    |                 | + | AnsibleAutomationPlatform 2.4の場合                       |           |              |                                                       |
      | インターフェース|                 |                                                               |           |              |                                                       |
      ...
      |                 |                 |                                                               |           |              |                                                       |
      |                 +--------+--------+---------------------------------------------------------------+-----------+--------------+-------------------------------------------------------+
      |                 | プロトコル      | Ansible Automation Controller/Platform Gateway\               | ー        | 手動入力     | :menuselection:`実行エンジン` が\                     |
      ...
      |                 |                 |                                                               |           |              |                                                       |
      |                 +--------+--------+---------------------------------------------------------------+-----------+--------------+-------------------------------------------------------+
      |                 | ポート          | Ansible Automation Controller/Platform Gatewayとの\           | ー        | 手動入力     | :menuselection:`実行エンジン` が\                     |
      |                 |                 | 接続プロトコルのポートを入力します。                          |           |              | Ansible Automation Controllerの場合に入力必須         |
      |                 |                 |                                                               |           |              |                                                       |
      |                 |                 | 通常はhttps（443）を入力します。                              |           |              |                                                       |
      |                 |                 |                                                               |           |              |                                                       |
      |                 |                 |                                                               |           |              |                                                       |
      |                 |                 |                                                               |           |              |                                                       |
      |                 +--------+--------+---------------------------------------------------------------+-----------+--------------+-------------------------------------------------------+
      |                 | 組織名          | Ansible Automation Controller/Platform Gateway\               | ー        | リスト選択   | :menuselection:`実行エンジン` が\                     |
      ...
      |                 |                 |                                                               |           |              |                                                       |
      |                 +--------+--------+---------------------------------------------------------------+-----------+--------------+-------------------------------------------------------+
      |                 | 認証トークン    | Ansible Automation Controller/Platform Gatewayに\             |           |              | 最大長255バイト                                       |
      |                 |                 | 接続するユーザの認証トークンを入力します。                    |           |              |                                                       |
      ...
      |                 | アウト値（単位\ | ITAからAnsible Automation Controller/Platform Gateway\        |           |              |                                                       |
      ...
      |                 |                 |                                                               |           |              | 大255バイト                                           |
      |                 |                 | ITAがプロキシ環境下にある場合、\                              |           |              |                                                       |
      |                 |                 | Ansible Automation Controller/Platform Gateway\               |           |              |                                                       |
      ...
      |                                   | Ansible Automation Platform又はAnsible Execution Agent内\     |           |              | Ansible Automation Controllerか\                      |
      |                                   | に一時的に生成したデータリソース\                             |           |              | Ansible Execution Agentの場合に入力必須               |
      |                                   | を作業終了後に削除するかを選択します。                        |           |              |                                                       |
      |                                   |                                                               |           |              |                                                       |
      |                                   | 「True」を選択した場合に削除します。                          |           |              |                                                       |
      |                                   |                                                               |           |              |                                                       |
      ...
      |                                   | を参照してください。                                          |           |              |                                                       |
      |                                   |                                                               |           |              |                                                       |
      |                                   | + | :menuselection:`実行エンジン` が「Ansible Core」か        |           |              |                                                       |
      |                                   |     「Ansible Execution Agent」の場合                         |           |              |                                                       |
      ...
      |                                   |   | ジョブテンプレートについての詳細は、\                     |           |              |                                                       |
      |                                   |     Ansible Automation Platform公式マニュアルの\              |           |              |                                                       |
      |                                   |     ユーザガイドを参照してください。                          |           |              |                                                       |
      |                                   |                                                               |           |              |                                                       |
...
#. | Ansible Automation PlatformへのRestAPI実行に必要な情報、および構築資材をAnsible Automation Platformに連携するために必要な情報のメンテナンス(閲覧/登録/更新/廃止)を行います。
   | クラスタ構成の場合は、構成している全てのノードを登録してください。
   | ただし、Ansible Automation Platformのhop nodeの登録は不要です。


   .. tip::
      | Ansible Automation Platform 2.5 では Platform Gateway・Execution Node(Hybrid Node含む)の登録を行います。
      | Controller Node・Hop Nodeの登録は不要です。

   .. figure:: /images/ja/ansible_common/ansible_automation_controller/ansible_automation_controller_host_list.png
      :width: 800px
      :alt: サブメニュー画面（Ansible Automation Platform ノード一覧）

      サブメニュー画面（Ansible Automation Platform ノード一覧）

#. | :guilabel:`登録` ボタンより、Ansible Automation Platformのノード情報の登録を行います。

#. | 登録画面の項目一覧は以下のとおりです。

| Ansible Automation Platformのバージョンによって設定する値が異なりますので、適切なバージョンを指定してください。

.. tabs::

   .. tab:: Ansible Automation Platform 2.4
      ...
      .. table:: 登録画面項目一覧（Ansible Automation Platform ノード一覧）
         ...
         |                                                     | Controller Node/Execution Node/Hybrid Nodeのホスト名\      |          |                 |                                          |
         |                                                     | (またはIPアドレス)を入力します。                           |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | 認証方式                                            | Execution Node/Hybrid Nodeへファイル転送(scp)で\           | ○        | リスト選択      | 説明欄記載のとおり。                     |
         |                                                     | 接続する際の認証方式を選択します。                         |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         |                                                     | + | パスワード認証                                         |          |                 |                                          |
         |                                                     |   | パスワードの入力が必須です。                           |          |                 |                                          |
         ...
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | ユーザ                                              | Execution Node/Hybrid Nodeへファイル転送(scp)で\           | ○        | 手動入力        | 最大長255バイト                          |
         |                                                     | 接続する際のログインユーザを入力します。                   |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         ...
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | パスワード                                          | Execution Node/Hybrid Nodeへファイル転送(scp)で\           | ○        | 手動入力        | 最大長400バイト                          |
         |                                                     | 接続する際のログインパスワードを入力します。               |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         |                                                     | 認証方式でパスワード認証を選択した場合、必須入力です。     |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         |                                                     | Ansible Automation Platformインストール時に生成される\     |          |                 |                                          |
         |                                                     | awxユーザに設定したパスワードを入力することを推奨します。  |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         +--------------------------+--------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | ssh鍵認証情報            | ssh秘密鍵ファイル        | Execution Node/Hybrid Nodeへファイル転送(scp)で\           | ー       | ファイル選択    | 最大サイズ100Mバイト                     |
         |                          |                          | 接続する際の秘密鍵ファイルを入力します。                   |          |                 |                                          |
         |                          |                          |                                                            |          |                 |                                          |
         ...
         |                          |                          |                                                            |          |                 |                                          |
         +--------------------------+--------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | ポート                                              | Execution Node/Hybrid Nodeにファイル転送(scp)で\           | ー       | 手動入力        | 数値                                     |
         |                                                     | 接続する際のポート番号を指定することができます。           |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         |                                                     | デフォルトは22が使用されます。                             |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | Execution node                                      | クラスタ構成で構築されている\                              | ー       | リスト選択      | 説明欄記載のとおり。                     |
         |                                                     | Ansible Automation Controllerで\                           |          |                 |                                          |
         |                                                     | 対象ホストが「execution node」の場合に「True」を\          |          |                 |                                          |
         |                                                     | 選択します。                                               |          |                 |                                          |
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
   ...
   .. tab:: Ansible Automation Platform 2.5
      ...
      .. table:: 登録画面項目一覧（Ansible Automation Platform ノード一覧）
         ...
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | 認証方式                                            | Execution Node(Hybrid Node)へファイル転送(scp)で\          | ○        | リスト選択      | Platform Gatewayはこの設定値を\          |
         ...
         |                                                     |                                                            |          |                 |                                          |
         +-----------------------------------------------------+------------------------------------------------------------+----------+-----------------+------------------------------------------+
         | ユーザ                                              | Execution Node(Hybrid Node)へファイル転送(scp)で\          | ○        | 手動入力        | 最大長255バイト                          |
         |                                                     | 接続する際のログインユーザを入力します。                   |          |                 |                                          |
...
.. tip:: グローバル変数名の一意性について

   「:menuselection:`Ansible共通 --> グローバル変数管理`」および「:menuselection:`Ansible共通 --> グローバル変数（センシティブ）管理`」において、
   **グローバル変数名** は一意である必要があります。
   これらのメニュー両方で同じ名称の :menuselection:`グローバル変数名` を登録することはできません。

.. _ansible_common_global_variable_list_for_sensitiv:

グローバル変数（センシティブ）管理
------------------------------------

#. | この機能では、Playbookや対話ファイルで共通して利用するグローバル変数の閲覧、登録、更新、廃止といったメンテナンスを行えます。
   | 通常のグローバル変数と同様に利用できますが、このメニューで登録されたグローバル変数は暗号化して保存されます。
   | Ansible実行時には、`ansible-vault` を通して安全に使用されます。

   .. figure:: /images/ja/ansible_common/global_variable/global_variable_list_for_sensitiv_v2-6.png
      :width: 800px
      :alt: サブメニュー画面（グローバル変数（センシティブ）管理）

      サブメニュー画面（グローバル変数（センシティブ）管理）

#. | :guilabel:`登録` ボタンより、グローバル変数の登録を行います。

   .. figure:: /images/ja/ansible_common/global_variable/global_variable_list_for_sensitiv_v2-6.gif
      :width: 800px
      :alt: 登録画面（グローバル変数（センシティブ）管理）

      登録画面（グローバル変数（センシティブ）管理）
...
#. | 登録画面の項目一覧は以下のとおりです。
   ...
   .. list-table:: 登録画面項目一覧（グローバル変数管理）
      ...
      * - 具体値
        -  | 具体値を入力します。複数行の具体値も入力できますが、Pioneerの対話ファイルで使用しているグローバル変数で複数行の具体値を設定すると、作業実行時にエラーとなります。
           | 入力された値は暗号化されて保存され、Ansible実行時には`ansible-vault` を通じて安全に使用されます。
           | 入力時には、入力欄右側の、 アイコンで入力内容の表示/非表示を切り替え、確認することができます。
           | 更新時、現在保存されている値は参照できませんのでご注意ください。
        - ー
        - 手動入力
        - 最大長4000バイト
      * - 変数名説明
...
.. tip:: | **大容量ファイルのファイル登録**
   | 大容量のファイルを送信する際に、「認証に失敗しました。」というエラーメッセージが表示されることがあります。
   | この事象は、以下のような複数の要因によって、ファイル登録の認証プロセスに遅延が発生し、アクセストークンの有効期限が切れたために発生します。
   | 　・ネットワーク回線の状態
   ...
   .. list-table:: 登録画面項目一覧（テンプレート管理）
      ...
      * - 変数定義
        - | テンプレートファイル素材内で使用している変数を定義します。
          | テンプレートファイル素材内で使用できる変数は5種類あります。※1
        - ー
        - 手動入力
        - 最大長4000バイト
      * - 備考
...
.. tip:: | ※1 **変数定義の注意事項**
   ...
   | テンプレートファイル内で使用できる変数は5種類あります。変数定義の注意事項は以下のとおりです。
...
.. _ansible_execution_environment_definition_template_list:

実行環境定義テンプレート管理
----------------------------

#. | Ansible Execution Agent内にansible-builderで実行環境（コンテナ）をbuildする際の実行環境定義ファイル(execution-environment.yml)のテンプレートファイルのメンテナンス(閲覧/登録/更新/廃止)を行います。
   | テンプレートファイルについての詳細は、「 :ref:`ansible_common_environment_definition_make` 」を参照してください。
   ...
   .. figure:: /images/ja/ansible_common/execution_environment_definition_template_list/execution_environment_definition_template_list.png
      :width: 800px
      :alt: サブメニュー画面（実行環境定義テンプレート管理）

      サブメニュー画面（実行環境定義テンプレート管理管理）

#. | :guilabel:`登録` ボタンより、実行環境定義テンプレート管理の登録を行います。
   ...
   .. figure:: /images/ja/ansible_common/execution_environment_definition_template_list/execution_environment_definition_template_list.gif
      :width: 800px
      :alt: 登録画面（実行環境定義テンプレート管理管理）

      登録画面（実行環境定義テンプレート管理管理）

#. | 登録画面の項目一覧は以下のとおりです。

   .. list-table:: 登録画面項目一覧（実行環境定義テンプレート管理）
      :widths: 20 80 10 10 10
      :header-rows: 1
      :align: left
      ...
      * - テンプレート名
        ...
        - 最大長255バイト
      * - テンプレートファイル
        - | ansible-builderで実行環境（コンテナ）をbuildする際に使用する実行環境定義ファイル(execution-environment.yml)のテンプレートファイルを入力します。
        - ○
        - ファイル登録
        - | Jinja2 template形式
          | 最大サイズ100Mバイト
...
#. | :menuselection:`Ansible共通 --> 実行環境定義テンプレート管理` より登録した実行環境定義ファイル(execution-environment.yml)のテンプレートファイルと :menuselection:`パラメータシート「実行環境定義テンプレート管理」` の紐付管理(閲覧/登録/更新/廃止)を行います。
   | :menuselection:`パラメータシート「実行環境定義テンプレート管理」` については「 :ref:`ansible_common_environment_definition_make` 」を参照してください。
   | 尚、ITAをインストールすると、実行環境定義テンプレートファイルに埋め込むパラメータが登録された :menuselection:`パラメータシート「実行環境パラメータ定義」` が登録され、
     :menuselection:`パラメータシート「実行環境パラメータ定義」` と :menuselection:`Ansible共通 --> 実行環境定義テンプレート管理` の紐付が登録されます。
...
#. | 登録画面の項目一覧は以下のとおりです。
   ...
   .. table:: 登録画面項目一覧（実行環境管理）
      ...
      | 項番                    | 登録時に自動採番した36桁の文字列が表示されます。                                                                            | ー    | 自動入力    | ー                            |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | 実行環境名              | 実行環境名を入力します。                                                                                                    | ○     | 手動入力    | 最大長255バイト               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | 実行環境構築方法        | 作業実行時にAnsible Execution Agent内でansible-builderで実行環境（コンテナ）をbuildするかどうかを選択します。               | ○     | リスト選択  | 説明欄記載のとおり。          |
      |                         |                                                                                                                             |       |             |                               |
      |                         | + | ITA                                                                                                                     |       |             |                               |
      |                         |   | 「ITA」を選択した場合には、ansible-builderで実行環境（コンテナ）をbuildします。                                         |       |             |                               |
      |                         |   | :menuselection:`実行環境定義名` と :menuselection:`テンプレート名` は必須入力です。                                     |       |             |                               |
      |                         | + | Manual                                                                                                                  |       |             |                               |
      |                         |   | 「Manual」を選択した場合には、ansible-builderで実行環境（コンテナ）をbuildしません。                                    |       |             |                               |
      |                         |   | ITA外でbulidした実行環境（コンテナイメージ）を作業実行前にAnsible Execution Agent内にロードしておく必要があれます。     |       |             |                               |
      |                         |                                                                                                                             |       |             |                               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | タグ名                  | ansible-builderで実行環境（コンテナ）をbuildされるイメージ名を入力します。                                                  | ○     | 手動入力    | 最大長255バイト               |
      |                         |                                                                                                                             |       |             |                               |
      |                         | + | :menuselection:`実行環境構築方法` で「ITA」を選択している場合                                                           |       |             |                               |
      |                         |   | イメージ名は以下の命名規則となります。                                                                                  |       |             |                               |
      |                         |   |   イメージ名の命名規則                                                                                                  |       |             |                               |
      |                         |   |   {{organization_id}}_{{workspace_id}}_"入力したタグ名"                                                                 |       |             |                               |
      |                         | + | :menuselection:`実行環境構築方法` で「Manual」を選択している場合                                                        |       |             |                               |
      |                         |   | イメージ名は入力したタグ名がイメージ名となります。                                                                      |       |             |                               |
      |                         |                                                                                                                             |       |             |                               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | 実行環境定義名          | :menuselection:`実行環境定義テンプレート管理` 用に作成されたパラメータシートに登録されている\                               | ○     | リスト選択  | 説明欄記載のとおり。          |
      |                         | :menuselection:`パラメータシート「実行環境パラメータ定義」` のメニュー名とレコード名が表示されます。\                       |       |             |                               |
      |                         | 一覧より選択します。                                                                                                        |       |             |                               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | テンプレート名          | :menuselection:`実行環境定義テンプレート管理` に登録されている :menuselection:`テンプレート名` が表示されます。             | ○     | リスト選択  | 説明欄記載のとおり。          |
      |                         | テンプレート名を選択します。                                                                                                |       |             |                               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
      | 備考                    | 自由記述欄です。                                                                                                            | ○     | 手動入力    | 最大長255バイト               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------+-------------+-------------------------------+
...
#. | ITAと接続したAnsible Execution Agentのエージェント名とバージョン情報を閲覧できます。
   ...
   .. figure:: /images/ja/ansible_common/ansible_agent_list/ansible_agent_list.png
      :width: 800px
      :alt: サブメニュー画面（エージェント管理）

      サブメニュー画面（エージェント管理）

#. | 閲覧画面の項目一覧は以下のとおりです。

   .. table:: 登録画面項目一覧（エージェント管理）
      :align: left

      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
      | 項目                    | 説明                                                                                                                        | 制約事項                      |
      +=========================+=============================================================================================================================+===============================+
      | 項番                    | 登録時に自動採番した36桁の文字列が表示されます。                                                                            | ー                            |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
      | エージェント名          | Ansible Execution Agentのエージェント名が表示されます。                                                                     | ー                            |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
      | バージョン              | Ansible Execution Agentのバージョンが表示されます。                                                                         | ー                            |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
      | ステータス              | ステータスには以下の状態が表示されます。                                                                                    | ー                            |
      |                         |                                                                                                                             |                               |
      |                         | + | 正常（最新）                                                                                                            |                               |
      |                         |   | Ansible Execution AgentのパーションとITAのバージョンが一致、Ansible Execution Agentと正常に連携が出来ている状態です。   |                               |
      |                         | + | 正常（更新可能）                                                                                                        |                               |
      |                         |   | Ansible Execution AgentのパーションとITAのバージョンが一致しないが互換性があるので、Ansible Execution Agent\            |                               |
      |                         |     と正常に連携が出来ている状態ですが、Ansible Execution Agentのバージョンアップを推奨します。                             |                               |
      |                         | + | 連携不可（要更新）                                                                                                      |                               |
      |                         |   | Ansible Execution AgentのパーションとITAのバージョンが一致しなく互換性がない為、Ansible Execution Agent\                |                               |
      |                         |     と連携が出来ない状態です。Ansible Execution Agentのバージョンアップを行ってください。                                     |                               |
      |                         |                                                                                                                             |                               |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
      | 備考                    | 自由記述欄です。                                                                                                            | 自由記述欄です。              |
      +-------------------------+-----------------------------------------------------------------------------------------------------------------------------+-------------------------------+
...
:menuselection:`Ansible共通 --> 実行環境定義テンプレート管理` に登録するテンプレートファイルと :menuselection:`パラメータシート「実行環境パラメータ定義」` の使用方法
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
.. table::   Inventory_host_parametersに設定するパラメータ一覧
   ...
   +                               +-----------------------+------------------------+------------------------+------------------------+-----------------------+-----------------------+                                                                                                                             +
   |                               | パスワード認証        | 鍵認証                 | 鍵認証                 | 鍵認証                 | パスワード認証        | 証明書認証            |                                                                                                                             |
   |                               |                       |                        |                        |                        |                       |                       |                                                                                                                             |
   |                               |                       | （パスフレーズなし）   | （パスフレーズあり）   | （鍵交換済み）         | 認証（winrm）         | （winrm）             |                                                                                                                             |
   +===============================+=======================+========================+========================+========================+=======================+=======================+=============================================================================================================================+
   | ansible_host                  | 〇                    | 〇                     | 〇                     | 〇                     | 〇                    | 〇                    | :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`DNSホスト` または :menuselection:`DNSホストIPアドレス`         |
   ...
   +-------------------------------+-----------------------+------------------------+------------------------+------------------------+-----------------------+-----------------------+-----------------------------------------------------------------------------------------------------------------------------+
   | ansible_port                  |                       |                        |                        |                        | 〇                    | 〇                    | :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`WinRM接続情報 --> ポート番号`                                  |
   +-------------------------------+-----------------------+------------------------+------------------------+------------------------+-----------------------+-----------------------+-----------------------------------------------------------------------------------------------------------------------------+
   | ansible_winrm_cert_pem        |                       |                        |                        |                        | 〇                    | 〇                    | :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`WinRM接続情報 --> winrm公開鍵ファイル`                         |
   +-------------------------------+-----------------------+------------------------+------------------------+------------------------+-----------------------+-----------------------+-----------------------------------------------------------------------------------------------------------------------------+
   | ansible_winrm_cert_key_pem    |                       |                        |                        |                        | 〇                    | 〇                    | :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`WinRM接続情報 --> winrm秘密鍵ファイル`                         |
   +-------------------------------+-----------------------+------------------------+------------------------+------------------------+-----------------------+-----------------------+-----------------------------------------------------------------------------------------------------------------------------+
