...
-  **作業フロー詳細と参照先**
   ...
   #. | **インターフェース情報の登録**
      | :menuselection:`Ansible共通 --> インターフェース情報` から、Ansible Core、Ansible Automation Controller、Ansible Execution Agentのいずれの実行エンジンを使用するかを選択し、実行エンジンのサーバへの接続情報を登録します。
      | 詳細は「 :ref:`ansible_common_interface_information` 」を参照してください。

   #. | **実行環境定義テンプレート管理の登録（必要に応じて実施）**
      | :menuselection:`Ansible共通 --> 実行環境定義テンプレート管理` から、Ansible Execution Agent内にansible-builderで実行環境（コンテナ）をbuildする際の実行環境定義ファイル(execution-environment.yml)のテンプレートファイルを登録します。
      | 詳細は「 :ref:`ansible_execution_environment_definition_template_list` 」や「 :ref:`ansible_common_environment_definition_make` 」を参照してください。
      | 尚、ITAをインストールすると、pythonモジュールやansible galaxyコレクションが追加できるテンプレートファイルが登録されます。

   #. | **パラメータシート「実行環境パラメータ定義」の登録（必要に応じて実施）**
      | :menuselection:`Ansible共通 --> 実行環境定義テンプレート管理` より登録した実行環境定義ファイル(execution-environment.yml)のテンプレートファイルに埋め込むパラメータを登録します。
      | 詳細は「 :ref:`ansible_common_environment_definition_make` 」を参照してください。
      | 尚、ITAをインストールすると、実行環境定義テンプレートファイル(execution-environment.yml)に埋め込むパラメータが登録された :menuselection:`パラメータシート「実行環境パラメータ定義」` が登録されます。
...
#. | **パラメータ確認**
   | :guilabel:`パラメータ確認` ボタンをクリックすると、実際に作業対象に対して構築作業をせず、 :menuselection:`Ansible-Pioneer --> 代入値自動登録設定` に登録してある情報から作業対象のオペレーションとMovementに紐付く情報を :menuselection:`Ansible-Pioneer --> 代入値管理` と :menuselection:`Ansible-Pioneer --> 作業対象ホスト` に反映し、確認することができます。
...
#. | **実行ログ表示**
   ...
   | 実行ログが分割された場合、実行ログがTabで分割表示され、表示したい実行ログを選択することができます。
...
#. | 閲覧画面の項目一覧は以下のとおりです。
   ...
   .. table:: 閲覧画面項目一覧（作業管理）
      ...
      +-------------------------------------------------------------------------------+------------------------------------------------------------------------------+
      | 緊急停止フラグ                                                                | :menuselection:`Ansible-Pioneer --> 作業状態確認` より :guilabel:`緊急\      |
      |                                                                               | 停止` ボタンで緊急停止をしているかを表示します。                             |
      +-------------------------------------------------------------------------------+------------------------------------------------------------------------------+
      ...
      |                     |                               +-------------------------+------------------------------------------------------------------------------+
      |                     |                               | ansible.cfg             | 作業実行で選択したMovementのansible.cfgがアップロードできます。              |
      |                     +-------------------------------+-------------------------+------------------------------------------------------------------------------+
      ...
      +---------------------+---------------------------------------------------------+------------------------------------------------------------------------------+
      | 投入データ                                                                    | 投入データ一式をzip形式でアップロードできます。                              |
      +-------------------------------------------------------------------------------+------------------------------------------------------------------------------+
      | 結果データ                                                                    | 結果データ一式をzip形式でアップロードできます。                              |
      +---------------------+---------------------------------------------------------+------------------------------------------------------------------------------+
...
.. table:: 対話モジュール一覧
  ...
  |                     | 後に条件分岐を行うことができます。                     |
  +---------------------+--------------------------------------------------------+
  | localaction         | Ansible Core/Ansible Automation Controller/Ansible \   |
  |                     | Execution Agentが実行される環境でコマンドを投入します。|
  +---------------------+--------------------------------------------------------+
...
| ⑥直後の処理でコマンドプロンプトの待ち受けができるようにリターンを投入する。
...
.. table:: commandモジュールの書式
   ...
   |                      |       で囲む                             |            | 　when:と同様の記述が行えます。                                                    |
   |                      |                                          |            |                                                                                    |
   |                      |                                          |            | commandに記載したコマンドの投入後の標準出力の内容を「stdout」で記述できます。      |
   |                      |                                          |            | 　stdout 比較演算子 文字列                                                         |
   |                      |                                          |            |                                                                                    |
   +----------------------+------------------------------------------+------------+------------------------------------------------------------------------------------+
...
| Ansible Core/Ansible AutomationController/Ansible Execution Agentが実行される環境でコマンドを投入します。
...
| 対話ファイルで複数具体値変数が使用できるパラメータは、commandモジュールのwith_itemsパラメータのみです。これ以外で使用した場合、作業実行時にエラーとなります。
...
| 一部の全角文字（ ①②等）をUTF-8にデコードできません。デコード出来ない文字は??で表示されます。
| 一部の全角文字（ － 等）をexpect等のコマンドプロンプト待ちで使用した場合、UTF-8では待ち受けが正しくできますが、euc/shift_jisでは待ち受けがタイムアウトしてしまいます。
...
.. list-table:: Ansible-Pioneer投入データ
   ...
   * - Ansible共通
     ...
     - :menuselection:`Ansible共通 --> インターフェース情報` の :menuselection:`実行エンジン` が「Ansible Execution Agent」の場合のみ
   * - Ansible共通
