...
#. | 登録画面の項目一覧は以下のとおりです。
   ...
   .. table:: 登録画面項目一覧（OS種別）
      ...
      |             +--------------+------------------------------------------------------------------------+--------------------+-----------------------+-------------------------------------------------+
      | 機器種別    | NW           | ネットワーク機器の場合「True」を選択します。                           | ー                 | リスト選択            | ー                                              |
      |             +--------------+------------------------------------------------------------------------+--------------------+-----------------------+-------------------------------------------------+
...
#. | **実行ログ表示**
   | Ansible Automation Controllerで実行した場合、作業対象の :menuselection:`Ansible共通 --> 機器一覧` の :menuselection:`ユーザ` ・ :menuselection:`パスワード` ・ :menuselection:`ssh秘密鍵ファイル` ・ :menuselection:`パスフレーズ` ・ :menuselection:`接続タイプ` ・ :menuselection:`インスタンスグループ` の項目値でグループ化された作業対象の単位で対話ファイルが実行され、ansibleの実行ログが分割されます。
...
- | e.g.）failed_whenを使用した例
  ...
  .. code-block:: yaml
     ...
     exec_list:
       ...
       # 「Active: active （running）」のrunningが一致するので正となります。（次のループに移ります）
