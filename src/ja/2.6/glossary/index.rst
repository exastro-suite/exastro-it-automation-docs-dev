...
.. glossary::
    ...
    Conductorインスタンス
        Conductorクラスを実行した際に生成される実行実体です。Conductorクラスの設計図に基づいて実際に動作し、実行状態や実行結果を保持します。

    Conductor作業履歴
        Conductorクラスの実行履歴を管理する機能です。各実行インスタンスの開始時刻、終了時刻、実行状態、実行結果、エラー情報などが記録されます。

    Conductorノードインスタンス
        Conductorクラスを実行する際に生成される、個々のステップ（ノード）の実行実体です。作業実行時にノードの状態管理、実行結果の保持、次ノードへの制御フローの受け渡しなどを行います。
