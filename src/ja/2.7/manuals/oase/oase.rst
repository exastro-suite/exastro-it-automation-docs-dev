...
.. list-table:: 用語説明
   ...
   * - ラベル
     - | 取得したイベントに対して付与するタグや分類情報のことです。
       ...
       | イベント取得時に自動的に付与されるシステムラベル（Exastro用ラベル）のほかに、
       | :menuselection:`OASE --> ラベル作成` および :menuselection:`OASE --> ラベル付与` でユーザーが定義したラベルを使用できます。
       |
       | システムラベル一覧
       | ・ _exastro_event_collection_settings_id：どのイベント収集設定で収集されたかを示すID
       | ・ _exastro_fetched_time：イベントが取得された日時
       | ・ _exastro_end_time：イベントの有効期限日時（TTL）
       | ・ _exastro_agent_name：イベントを収集したエージェントの名前
       | ・ _exastro_agent_version：イベントを収集したエージェントのバージョン
       | ・ _exastro_type：イベントの種別（通常イベント/結論イベント）
       | ・ _exastro_checked：新規イベント通知が行われたかどうかのフラグ（0：未、1：済）
       | ・ _exastro_evaluated：イベントがルール評価済みかどうかのフラグ（0：未、1：済）
       | ・ _exastro_undetected：イベントが未知かどうかのフラグ（0：通常、1：未知）
       | ・ _exastro_timeout：イベントが時間切れかどうかのフラグ（0：有効、1：時間切れ）
       |
       | 例）ユーザー定義のラベル
       | ・ severity：イベントの重要度
       | ・ host：ホスト名   
       | ・ environment：イベントが発生した環境
       | 
   * - フィルター
     - | ラベル付与されたイベントの中から特定の条件に合致するイベントだけを抽出する仕組みです。
       | ルール判定の前段階で、どのイベントをルール判定の対象にするかを絞り込む役割を果たします。
       | 
       | 例）以下の条件を満たすイベントを抽出する
       | ・ severityラベルが"critical"と等しい
       | ・ metricラベルが"CPU"と等しい
       | ・ thresholdラベルが"90"と等しい
       | 
   * - ルール
     - | フィルターとアクションを組み合わせたもので、イベントに対する具体的な対応策を定義します。
       | フィルターで絞り込まれたイベントに対してどのような条件でどんなアクションを実行するかを定義するものです。
       | 
       | 例）
       | ・ CPU使用率が90%を超えた場合に、管理者にメール通知を送る
       | ・ ディスク容量が残り10%を切った場合に、不要なファイルを自動削除する
       | 
   * - アクション
     - | 定義されたルールにマッチした場合に実行される対象です。
       ...
       | 
   * - 評価
     - 収集したイベントがルールの条件に一致するかどうかを判定し、一致した場合にアクションを実行するまでの一連のプロセスのことです。
       | 
   * - 結論イベント
     - | ルールにマッチしたイベントの結果として生成される新しいイベントです。
       | 元のイベントから情報を引き継ぎ、処理結果や判定結果を含むことができます。
       | 結論イベントも評価対象となるため、連鎖的な自動化が可能です。
       | 
       | 例）
       | CPU使用率が90%を超えたイベントに対して、管理者にメール通知を送るアクションが実行された場合、
       | その結果として「メール通知が成功した」または「メール通知が失敗した」という結論イベントが生成される。
       | 

イベント処理イメージ
---------------------

| 監視システムが検知したシステムの状態変化をイベントとして収集し、評価を行います。
| 評価では、フィルターでイベントを絞り込み、ルールで条件に一致するかを判定し、一致した場合にアクションを実行します。
| 以下に、OASEのシステム処理イメージを示します。

.. figure:: /images/ja/oase/oase/oase_event_flow.png
   :alt: OASEイベント処理フロー
...
.. list-table::
   ...
   * - 検索方法
     - | ラベルの検索方法を選択します。
       ...
       | キューイング：一意のイベントを抽出しますが、複数イベントがヒットした場合、先頭のイベントを使用します。ルールに複数回マッチする可能性があるため、ご注意ください。
...
検索方法の詳細は、:ref:`filter_search_method` を参照してください。
...
| フィルター条件にヒットする一意のイベントの抽出を行います。
| 複数イベントがヒットした場合、ヒットしたすべてのイベントを未知のイベントとして処理します。
...
| 複数のイベントが同じグループにグルーピングされた場合、先頭のイベントを使用し、その他のイベントは無視され、先頭イベントのみイベントフロー画面に表示されます。

| グルーピングの対象となるラベルは以下です。
| - _exastro_host
| - ラベル作成で作成したラベル
