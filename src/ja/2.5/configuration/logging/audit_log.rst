...
| 対象期間の設定値を :guilabel:`カレンダーボタン` から指定し、画面下の :guilabel:`ダウンロード` を押下することで監査ログのダウンロードが開始されます。

.. figure:: /images/ja/log/audit_log/audit_log_target_period.png
    :alt: 監査ログ対象期間
    :width: 900px

| :menuselection:`履歴` タブでは、ダウンロードした監査ログの履歴が表示され、再度ダウンロードすることができます。
| また、監査ログのダウンロード履歴は7日後に削除され、ダウンロード最大件数は100件までとなります。
...
.. list-table:: 監査ログ履歴項目一覧
   ...
   * - ステータス
     - | 監査ログをダウンロードするための処理のステータスが表示されます。ステータスは以下の種類があります。
       | NotExecuted (未実行)
       | Executing (実行中)
       | Completion (完了)
       | Failed (失敗)
