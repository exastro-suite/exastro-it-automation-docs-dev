...
.. warning::
   | 以下のサービスアカウントは、Ansible実行エンジンで、「Ansible Automation Platform (Cloud)」を設定している場合の作業実行で使用されます。

   - aap_service_account_user_<organization_id>_<workspace_id>

   | 作業実行時に、該当サービスアカウントの確認を行い、無い場合は作成を行います。
   | サービスアカウント・リフレッシュトークンを削除することで実行中の作業に影響が出ますので、削除実施する場合は十分に注意したうえで実施してください。
