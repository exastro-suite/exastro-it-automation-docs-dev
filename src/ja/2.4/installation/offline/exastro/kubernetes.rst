...
| 「["x.x.x"]="x.x.x"」にはExastro IT Automation App VersionとExastro Platform App Versionをそれぞれ記載します。
| IPaddressはAnsible実行環境として使用するサーバのIPアドレスを指定します。
...
| 収集した資材をFTP、SCP、SFTP、記憶媒体等でオフライン環境に転送します。
| オフライン環境の/tmpに/workディレクトリを作成し、以下の資材を配置します。
...
.. tabs::
   ...
   .. group-tab:: AlmaLinux						
      ...
      .. code-block:: shell
         ...
         sudo tee /etc/yum.repos.d/pkg-repo.repo <<EOF
         [pkg-repo]
         name=AlmaLinux-\$releaserver - pkg
         baseurl=file:///tmp/pkg-repo
         enabled=1
         gpgcheck=0
         ...
         sudo dnf install -y --disablerepo=\* --enablerepo=pkg-repo ${docker_ce}
          
         #pythonのインストール
         python=$(ls | grep "python39")

         sudo dnf install -y --disablerepo=\* --enablerepo=pkg-repo ${python}
...
| 1行を追記します。xx.xx.xx.xxには現在作業しているサーバ(Ansible実行環境)のIPアドレスを指定します。
...
| 作成された :file:`hosts.yml` を構成したいKubernetesクラスターのホスト名に書き換えます。
| 以下の記載例はKubernetesクラスターを1台で構成した例です。
| 本説明では、各Kubernetesクラスターがコントロールプレーン、ワーカーノードを兼ねた設定で作成するため、全てのhostnameに同じホスト名を指定します。
...
| Exastroのインストール時にエラーが発生した場合は一度アンインストールし、永続ボリュームとkubernetesの削除を行った後、再インストールします。
