...
1. | イベント収集※1 画面の入力項目は以下のとおりです。
   ...
   .. table:: イベント収集画面 入力項目一覧
      ...
      |                                    | を考慮した、それ以下の階層を指定します。               |              |              |                 |
      +------------------------------------+--------------------------------------------------------+--------------+--------------+-----------------+
      | TTL                                | TTL（Time To Live）とは、エージェントが取得した\       | 〇           | 手動入力     | 最小値10（秒）  |
...
| 　　　　　　上記の 1. 配列の値を取得するJMESPath や、3. 深い階層で、複数の値を取得するJMESPath の様な場合
...
.. warning::
   | 監視ソフトのバージョンによって、HTTP APIの仕様が異なる場合があります。
...
.. note::
  ...
  | 但し、下記のコマンドを実行すると、既に存在するユーザーで作成済みの<Zabbix APIトークン>が使用できなくなる場合があります。
  | 対応として、新しいユーザーを作成し、新しいユーザーで<Zabbix APIトークン>を作成することをお勧めします。

  | 新しいユーザーの作成は、

  1. | ブラウザで、:dfn:`Zabbix` に管理者でサインイン
     | 　初期状態の管理者のログイン情報は、
     | 　　・ユーザー名： Admin
     | 　　・パスワード： zabbix
...
.. tip::
   | 埋め込むデータの中に TAB(\\x09)、LF(\\x0a)、CR(\\x0d)以外の ASCIIの制御コード（\\x01～\\x1f）が含まれる場合、Exastro IT Automation でエラーとなります。（エラー内容は platform-job サービスのログに出力されます）
   | そのため、通知テンプレートに以下のフィルターを入れることでエラーを回避することができます。
   | 以下は \\x01 を空文字に変換する例です。エラーとなる他の制御コードも同様に変換してください。

   .. code-block:: none

      {
         "data_field": "{{ data_variable | replace('\x01', '') }}"
      }
...
- | 2.新規イベント（統合予定）のテンプレート例: New(consolidated).j2
...
.. figure:: /images/ja/oase/oase_management/snow_incident_sample_02_merged.drawio.png
  :width: 800px
  :alt: 新規イベント（統合予定）: ServiceNow(インシデント)
