...
| 評価では、フィルターでイベントを絞り込み、ルールで条件に一致するかを判定し、一致した場合に次のアクションを決定、実行します。
...
.. figure:: /images/ja/oase/oase/oase_eventflow_example.png
   :alt: Webサーバー停止時の自動再起動処理フロー
...
.. list-table:: 用語説明
   ...
   * - OASE
     - | Exastro OASEは、システムの監視・運用自動化を支援するソフトウェアです。
       ...
       | 
   * - イベント
     - | 監視システムが検知した監視対象の状態変化を、Exastroに通知するための情報のことです。
       | 
   * - ラベル
     - | 取得したイベントに対して付与するタグや分類情報のことです。
       | イベントを整理・分類し、後続のフィルターやルール判定で使いやすくするために使用します。
       | ラベルによって、異なるシステムからのイベントを均質化した情報として扱うことが可能です。
       | 
   * - フィルター
     - | ラベル付与されたイベントの中から特定の条件に合致するイベントだけを抽出する仕組みです。
       | ルール判定の前段階で、どのイベントをルール判定の対象にするかを絞り込む役割を果たします。
       | 
   * - ルール
     - | フィルターで絞り込まれたイベントに対して、どのような条件でどのアクションを実行するかを定義するものです。
       | 本定義は、フィルターとアクションを紐づける役割を持ちます。
       | 
   * - 評価
     - | 収集したイベントがフィルターによって抽出され、ルールの条件に一致するかどうかを判定し、
       | 一致した場合に次の動作（アクション、結論イベント）を決定するまでの一連のプロセスのことです。
       | 
   * - アクション
     - | 定義されたルールにマッチした場合に、イベントに対して実行する具体的な対応処理です。
       | 
   * - 結論イベント
     - | ルールにマッチしたイベントの結果として生成される新しいイベントです。
...
.. figure:: /images/ja/oase/oase/oase_rure_process.png
   :width: 700px
   :alt: OASE作業フロー
...
-  **作業フロー詳細と参照先**
   ...
   #. | **ラベルの登録**
      ...
      | 詳細は :ref:`label_creation` 、:ref:`labeling` を参照してください。
...
| :menuselection:`OASE --> イベントフロー` 画面では、特定のイベントの評価処理の流れを可視化する画面です。
...
.. raw:: html
   ...
   <style type="text/css">
      #eventflowDisplayTable table td {
         vertical-align: top;
      }
   </style> 

   <div id="eventflowDisplayTable">
...
.. list-table:: イベントフロー画面 表示パターン選択
   ...
   * - 新規イベント
     ...
     - .. image:: /images/ja/oase/oase/eventflow_icon_new.png
           :width: 50px
           :alt: 新規イベント アイコン
   * - 既知イベント
     - | 評価機能に検知された状態もしくは対象のことです。
     -  .. image:: /images/ja/oase/oase/eventflow_icon_known.png
          :width: 50px
          :alt: 既知イベント アイコン
   * - 未知イベント
     - | フィルターに抽出されなかった（評価機能に検知されなかった）状態もしくは対象のことです。※
       | 未知の事象であることから、今後の評価対象として検討する必要が考えられます。
     -  .. image:: /images/ja/oase/oase/eventflow_icon_unknown.png
          :width: 50px
          :alt: 未知イベント アイコン
   * - 時間切れイベント
     ...
     -  .. image:: /images/ja/oase/oase/eventflow_icon_timeout.png
          :width: 50px
          :alt: 時間切れイベント アイコン
   * - 結論イベント
     - ルールにマッチした際に、発生するイベントのことです。
     - .. image:: /images/ja/oase/oase/eventflow_icon_conclusion.png
          :width: 50px
          :alt: 結論イベント アイコン
   * - 実行アクション
     - ルールにマッチしたことにより実行したアクションの情報を表示します。
     - .. image:: /images/ja/oase/oase/eventflow_icon_action.png
          :width: 50px
          :alt: 実行アクション アイコン
   * - ルール
     - ルールID、ルール名を表示します。
     - .. image:: /images/ja/oase/oase/eventflow_icon_rule.png
          :width: 50px
          :alt: ルール アイコン
...
| :menuselection:`フィルター`・:menuselection:`アクション`・:menuselection:`ルール` について、各画面でも、イベントフロー内でもメンテナンス（閲覧/登録/更新/廃止）できます。ドラッグ&ドロップも使用できます。
...
1. | :menuselection:`OASE --> イベント履歴` 画面は、収集されたイベントを一覧で表示し、検索・フィルタリングできる画面です。
...
.. _label_creation_systemlabel:

..  list-table:: システムラベル一覧
   :widths: 40 60
   :header-rows: 1
...
| :menuselection:`OASE --> ラベル作成` 画面では、ユーザーラベルのキーを定義することができます。
| :menuselection:`OASE --> ラベル付与` 画面では、受信したイベントの特定フィールドの値を条件に、ラベルに固定値や特定フィールドの値を設定できます。
| 
...
1. | :menuselection:`OASE --> ラベル作成` 画面は、イベントで使用するラベルの定義（ラベルキー、データ型、説明など）を作成・管理する画面です。
...
1. | :menuselection:`OASE --> ラベル付与` 画面は、収集されたイベントに対して自動的にラベルを追加・更新する設定を作成する画面です。
...
重複排除について
^^^^^^^^^^^^^^^^^
| 重複排除機能は、同じ内容のイベントが重複して発生した場合に、重複するイベントを除外して処理負荷を軽減する機能です。
| イベント収集設定で重複排除を有効にすると、重複排除で指定したラベルキーの値が一致するイベントを重複とみなします。
| 例えば、「host」と「status」をキーに指定した場合、同じホストで同じ状態のイベントが複数回発生しても、最初の1件のみが処理対象となります。
| 
| エージェントを冗長化している環境では、複数のエージェントが同じイベントを送信することがあり、重複排除機能でそれらを1件にまとめることができます。
| 重複排除されたイベントは評価フローにも投入されないため、不要なアクション実行を防げます。

.. figure:: /images/ja/oase/oase/oase_deduplication_example.png
   :width: 800px
   :alt: 重複排除の例
...
| 重複排除機能を適切に設定することで、イベント数を削減し、システムリソースの消費を抑えながら、必要な対応は確実に実行できます。

重複排除画面
^^^^^^^^^^^^^

1. | :menuselection:`OASE --> 重複排除` 画面では、重複排除の設定をメンテナンス（閲覧/登録/更新/廃止）できます。
...
| :menuselection:`OASE --> フィルター` 画面では、複数のラベル条件を定義し、すべての条件に一致するイベントを抽出します。
| 
| フィルターで絞り込まれたイベントは、ルール評価の対象となります。
| 1つのフィルターは複数のルールで再利用できます。

.. _filter_search_method:
...
| フィルター条件に一致する一意のイベントの抽出を行います。
| 複数イベントがヒットした場合、ヒットしたすべてのイベントは未知のイベントとして処理されます。

.. figure:: /images/ja/oase/oase/filter_unique.png
...
| フィルター条件に一致するイベントを保持し、TTL時間内に収集された複数のイベントをまとめて評価します。

.. figure:: /images/ja/oase/oase/filter_queing.png
...
| 対象のラベルの値がすべて一致するイベントを１グループとしてグルーピングします。
| 複数のイベントが同じグループにグルーピングされた場合、先頭のイベントを使用し、その他のイベントは無視されます。
| イベントフロー画面には、先頭イベントのみ表示されます。

.. figure:: /images/ja/oase/oase/filter_grouping.png

   検索方法（グルーピング）

.. figure:: /images/ja/oase/oase/filter_grouping_eventflow.png
   :width: 800px

   イベントフロー画面（グルーピングの表示）


| グルーピングのフィルターでは指定したラベルの値とTTLでグループ化され、グループ毎に１回だけアクションが実行されます。

| グルーピングのイメージは以下です。

.. figure:: /images/ja/oase/oase/filter_grouping_image.png
   :width: 900px
...
1. | :menuselection:`OASE --> フィルター` 画面では、フィルターをメンテナンス（閲覧/登録/更新/廃止）できます。
...
.. list-table::
   ...
   * - 検索方法
     - | ラベルの検索方法を選択します。
       | ・ユニーク：条件に一致する一意のイベントを抽出します。複数イベントが同時にヒットした場合、ヒットしたイベントはすべて評価対象外（未知イベント）となります。
       | ・キューイング：条件に一致するイベントを保持し、TTL時間内に収集された複数のイベントをまとめて管理します。複数イベントがヒットした場合、先頭のイベントをルール評価の対象として使用します。
       | ・グルーピング：対象のラベルの値がすべて一致するイベントを１グループとしてグルーピングします。複数イベントがグルーピングされた場合、先頭のイベントを使用し、その他のイベントは無視（集約）されます。
     - 〇
     - 手動入力
     - ー
   * - ラベル（グルーピング対象）
...
| :menuselection:`OASE --> アクション` 画面では、Conductor、オペレーション、対象ホストなどを指定し、実行する作業内容を定義します。
